...
export class IntakeFormPage extends Component<IntakeFormPageProps,IntakeFormPageState> {
    ...
    onNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        this.props.dispatch(updateForm({name: e.target.value} as any))
    }
    onEmailChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        this.props.dispatch(updateForm({email: e.target.value} as any))
    }
    onAgeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        this.props.dispatch(updateForm({age: e.target.value} as any))
    }
    onSchoolChange = (e: React.ChangeEvent<any>) => {
        switch(e.target.value){
            case University.Carleton: 
            case University.Ottawa:
                this.props.dispatch(updateForm({city: 'Ottawa'} as any));
                break;
            case University.Toronto :
            case University.Ryerson:
            case University.York:
                this.props.dispatch(updateForm({city: 'Toronto'} as any));
                break;
            case University.Queens:
                this.props.dispatch(updateForm({city: 'Kingston'} as any));
                break;
            case University.Waterloo:
                this.props.dispatch(updateForm({city: 'Waterloo'} as any));
                break;
            default:
                this.props.dispatch(updateForm({city: ''} as any));
        }
        this.props.dispatch(updateForm({school: e.target.value} as any))
    }
    modifyTask = (e: React.ChangeEvent<HTMLInputElement>) => {
        const updates = {
            [e.target.value]: e.target.checked
        }
        this.props.dispatch(updateForm({tasks: updates} as any))
    }
